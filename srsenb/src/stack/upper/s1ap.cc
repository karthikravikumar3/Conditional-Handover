...
const init_msg_s& msg;
void create_x2ap_connection();

int32_t enbid ; 
int sockfd_m, connfd_m, sockfd1_m;
//int  *connfd, *sockfd1;
...
void send_message_s(int *connfd)
{
 char buff[MAX];
 char server_buffer[MAX] = {
     };
    char client_buffer[MAX] = {
        
    ...
    for (;;) {
        ...
        temp = read(*connfd, buff, sizeof(buff));
       ...
       // printf("From client: %s\t To client : ", buff);
        ...
        temp = write(*connfd, client_buffer, sizeof(buff));
  ...
  srsran::console("X2AP Handover started %llu\n ",ms_x2_Start);
                n++;
            }
        }
    }
...
void send_message_c(int *sockfd)
{
    int n;
    ...
    for (;;) {
        ...
        temp = write(*sockfd, client_buffer, sizeof(client_buffer));
...
void *server_init(void *sockfd_v)
{
    srsran::console(" Server Created %llu \n",enbid);
    int* sockfd = (int*)sockfd_v;
    ...
    *connfd = accept(*sockfd, (SA*)&cli, &len);
    ...
    else
    {
        ...
        send_message_s (connfd);
    }
    return(NULL);
}


void *client_init(void *sockfd1_v)
{
      srsran::console(" Client Created %llu \n",enbid);

    int* sockfd1 = (int * )(sockfd1_v);
    ...
    if (connect(*sockfd1, (SA*)&servaddr, sizeof(servaddr))
        != 0) {
...
void create_x2ap_connection()
{
    ...
    pthread_create(&thread_server, NULL, server_init , (void * ) sockfd);
    pthread_create(&thread_client, NULL, client_init , (void *) sockfd1);
